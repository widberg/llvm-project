...
X86RegisterInfo::getCalleeSavedRegs(const MachineFunction *MF) const {
  ...
  case CallingConv::PreserveMost:
    return IsWin64 ? adjust_regmask(CSR_Win64_RT_MostRegs_SaveList)
                   : adjust_regmask(CSR_64_RT_MostRegs_SaveList);
...
X86RegisterInfo::getCallPreservedMask(const MachineFunction &MF,
  ...
  case CallingConv::PreserveMost:
    return IsWin64 ? adjust_regmask(CSR_Win64_RT_MostRegs_RegMask)
                   : adjust_regmask(CSR_64_RT_MostRegs_RegMask);
  case CallingConv::PreserveAll:
    if (HasAVX)
